                                 ...
                                 const struct mf_field *mf)
{
    ...
    for (i = 0; i < mf->n_bits; i++) {
        uint16_t value = hs->move_map[ofs * 8 + mf->n_bytes * 8 - 1 - i];
        ...
        if (value != BIT_SET && value != BIT_UNSET) {
            ...
            uint16_t orig_map_value;
            uint8_t orig_mf_id, orig_bit;
            size_t orig_ofs;
            size_t idx;

            orig_mf_id = ((uint8_t *) &value)[0];
            orig_bit = ((uint8_t *) &value)[1];
            orig_mf = mf_from_id(orig_mf_id);
            orig_ofs = hsa_field_offset_from_mf(mf);
            idx = orig_ofs * 8 + orig_mf->n_bytes * 8 - 1 - orig_bit;
            orig_map_value = hs->move_map[idx];

            /* TODO: fix this. */
            /* Does not support BIT_SET, since no idea if the bit
             * is copied before/after the set. */
            ovs_assert(orig_map_value == BIT_UNSET);
            ...
            bitwise_copy(&cur_value, mf->n_bytes, i,
                         &orig_value, orig_mf->n_bytes, orig_bit, 1);
            bitwise_copy(&cur_mask, mf->n_bytes, i,
                         &orig_mask, orig_mf->n_bytes, orig_bit, 1);
