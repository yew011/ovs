...
register_gateway(struct controller_gw_ctx *ctx)
{
    ...
    SBREC_CHASSIS_FOR_EACH(chassis_rec, ctx->ovnsb_idl) {
        if (!strcmp(chassis_rec->name, ctx->chassis_id)) {
            VLOG_INFO("chassis already exists");
            break;
        }
    }
...
/* Updates the gateway_ports map in chassis table. */
static void
update_physical_ports(struct controller_gw_ctx *ctx)
{
    ...
    for (i = 0; i < chassis_rec->n_gateway_ports; i++) {
        if (sset_find_and_delete(&sset, chassis_rec->key_gateway_ports[i])) {
            *pp_names++ = xstrdup(chassis_rec->key_gateway_ports[i]);
            *gws++ = chassis_rec->value_gateway_ports[i];
        } else {
            changed = true;
        }
    }
    ...
    if (changed || !sset_is_empty(&sset)) {
        ...
        SSET_FOR_EACH (iter, &sset) {
            *pp_names++ = xstrdup(iter);
            *gws++ = sbrec_gateway_insert(txn);;
            sbrec_gateway_set_attached_port(gws[-1], pp_names[-1]);
        }
...
/* Updates the vlan_map in gateway table. */
static void
update_vlan_map(struct controller_gw_ctx *ctx)
{
    ...
    /* hmap must be empty, since we update all 'gateway_port's in
     * previous step. */
