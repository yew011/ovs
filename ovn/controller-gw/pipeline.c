 ...
 * returns false.*/
static bool
get_binding_from_map(const struct sbrec_bindings *binding, struct hmap *ls_map,
                     struct lswitch **lsp)
{
    struct lswitch *ls;
...
find_vtep_lswitch(struct controller_gw_ctx *ctx, const struct uuid *uuid)
{
    ...
    VTEPREC_LOGICAL_SWITCH_FOR_EACH (vlswitch, ctx->vtep_idl) {
        ...
        if (!memcmp(&uuid_, uuid, sizeof *uuid)) {
            return vlswitch;
        }
    }
    return NULL;
                     ...
                     const struct vteprec_logical_switch *vlswitch)
{
    ...
    uuid_from_string(&uuid, vlswitch->name);
    HMAP_FOR_EACH_WITH_HASH (ls, hmap_node, uuid_hash(&uuid), ls_map) {
        if (uuid_equals(&ls->ldp_uuid, &uuid)) {
            return ls;
        }
    }
...
/* Given chassis name, returns the 'struct sbrec_chassis *'. */
static const struct sbrec_chassis *
get_chassis_by_name(struct controller_gw_ctx *ctx, const char *name)
{
    const struct sbrec_chassis *chassis_rec;

    SBREC_CHASSIS_FOR_EACH(chassis_rec, ctx->ovnsb_idl) {
        if (!strcmp(chassis_rec->name, name)) {
            break;
        }
    }
...
collect_bindings(struct controller_gw_ctx *ctx)
{
    ...
    SBREC_BINDINGS_FOR_EACH (binding_rec, ctx->ovnsb_idl) {
        ...
        if (get_binding_from_map(binding_rec, other_lswitches, &ls)) {
            /* Do not care if 'binding_rec' is not on a logical switch
             * specified in vtep. */
            continue;
        } else if (get_binding_from_map(binding_rec, vtep_lswitches, &ls)) {
        ...
        HMAP_FOR_EACH (mb, hmap_node, &ls->bindings) {
            ...
            if (get_umr_by_mac(ucast_macs_rmts, mb->mac, &umr)) {
                ...
                if (mb->binding->chassis[0]) {
                    ...
                    chassis = get_chassis_by_name(ctx, mb->binding->chassis);
