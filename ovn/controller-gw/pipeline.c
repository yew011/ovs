...
struct lswitch {
    ...
    struct hmap mb_map;       /* stores 'struct mac_to_binding's. */
};
...
/* If there is already an 'mb' entry for 'mac' in 'ls->mb_map',
...
/* Creates an entry in 'ls->mb_map' for each mac in 'binding'. */
static void
add_binding_macs(struct lswitch *ls, const struct sbrec_binding *binding)
{
    ...
    for (i = 0; i < binding->n_mac; i++) {
        ...
        if (get_mb_from_lswitch(ls, mac, &mb)) {
            ...
            mb->mac = mac;
            mb->binding = binding;
            hmap_insert(&ls->mb_map, &mb->hmap_node, hash_string(mb->mac, 0));
        }
    }
}
...
lswitch_destroy(struct lswitch *ls)
{
    ...
    HMAP_FOR_EACH_SAFE (mb, next, hmap_node, &ls->mb_map) {
        hmap_remove(&ls->mb_map, &mb->hmap_node);
        free(mb);
    }
}
...
pipeline_run(struct controller_gw_ctx *ctx)
{
    ...
    HMAP_FOR_EACH (ls, hmap_node, vtep_lswitches) {
        ...
        HMAP_FOR_EACH (mb, hmap_node, &ls->mb_map) {
