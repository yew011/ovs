...
test_hash_main(int argc OVS_UNUSED, char *argv[] OVS_UNUSED)
{
     ...
     * 17-bit consecutive runs.
     *
     * Given a random distribution, the probability of at least one collision
     * in any set of 17 bits is approximately
     *
     *                      1 - ((2**17 - 1)/2**17)**C(129,2)
     *                   == 1 - (131,071/131,072)**8256
     *                   =~ 0.061
     *
     * There are 111 ways to pick 17 consecutive bits in a 128-bit word, so if
     * we assumed independence then the chance of having no collisions in any of
     * those 17-bit runs would be (1-0.061)**111 =~ 0.00092.  This refutes our
     * assumption of independence, which makes it seem like a good hash
     * function.
     */
    check_hash_bytes128(hash_bytes128, "hash_bytes128", 17);
